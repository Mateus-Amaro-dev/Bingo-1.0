...
function principal(){ //FAZ A CHAMADA DAS FUNÇOES
    ...
    randomBola();
    atualiza();
    timerGame = setInterval(randomBola, 3000);
...
function criarCartela(){
    ...
    for(let i=0; i<qtdNumeros; i++){
      var boalAtualRandom = randomNumeroBola();
...
function randomBola(){
        ...
        while(arrayBolasJogadas.length < qtdNumeros){
          bolaRandom = randomNumeroAtual();
            if(arrayBolasJogadas.indexOf(bolaRandom) === -1){
                    arrayBolasJogadas.push(bolaRandom);
            }
        }
          indiceBola++;
...
function endGame(){
    if(indiceEndGame == qtdNumerosCartela){
       ...
       nomeGanhador.innerHTML = nomeUsuario[nome()];
    }
